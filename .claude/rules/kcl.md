...
1. **`apis/<api>/composition.yaml` is GENERATED.** Edit `apis/<api>/kcl/main.k`, then
   `task generate`. CI fails if the inlined copy is stale.
2. **Always `kcl fmt`.** `task test` formats in place and fails on a dirty diff (`kcl fmt` has no
...
6. **Validate with `task check`** before committing.
